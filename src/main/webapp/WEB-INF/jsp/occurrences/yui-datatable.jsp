...
<c:set var="pageSize">20</c:set>
                  <script type="text/javascript">
                        ...
                        function loadDatatable(JsonUrl, thisQuery, thisFacetQuery) {
                            ...
                            var formatTitleUrl = function(elCell, oRecord, oColumn, sData) {
                                ...
                                elCell.innerHTML = "<a href='" + "${pageContext.request.contextPath}/occurrences/"
                                    + oRecord.getData("id") + "' title='view record details'>"
                                    + cellContent + "</a>";
                            };

                            myDataSource = new YAHOO.util.DataSource(JsonUrl+"&q="+thisQuery+"&fq="+thisFacetQuery+"&");
                            myDataSource.responseType = YAHOO.util.DataSource.TYPE_JSON;
                            myDataSource.responseSchema = {
                                resultsList: "response.docs",
                                fields: ["id","data_resource","institution_code","collection_code","catalogue_number", "taxon_name", "basis_of_record", "score"],

                                metaFields: {
                                    totalRecords: "response.numFound",
                                    paginationRecordOffset : "responseHeader.params.start",
                                    sortKey: "responseHeader.params.sort",
                                    sortDir: "responseHeader.params.sort"
                                }
                            };
                            ...
                            var myColumnDefs = [
                                ...
                                {key:"id", field:"data_resource", sortable:false, label:"Occurrence Record", formatter:formatTitleUrl}, // width: 350 , formatter:formatTitleUrl
                                //{key:"acceptedConceptName", field:"acceptedConceptName", sortable:false, label:"is synonym for", width: 350},
                                {key:"taxon_name", sortable:false, label:"Scientific Name"},
                                //{key:"highlight", field:"highlight", sortable:false, label:"Snippet"},
                                {key:"basis_or_record", label:"Type", sortable: true}, // , minWidth: 100,width: 200
                                {key:"score", hidden:true, maxAutoWidth: 0}
                            ];
                            ...
                            var generateRequest = function(startIndex,sortKey,dir,results) {
                                ...
                                return "rows="+results+"&start="+startIndex+"&sort="+sortKey+" "+dir;
                            };
