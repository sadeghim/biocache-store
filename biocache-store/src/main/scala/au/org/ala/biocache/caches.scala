...
object TaxonSpeciesListDAO {
  ...
  def getListsForTaxon(conceptLsid:String, forceLoad:Boolean):(List[String], Map[String,String])={
    val response = WebServiceLoader.getWSStringContent(Config.listToolUrl + "/species/" + conceptLsid)
    if(response != ""){
      ...
      list.foreach(map => {
        ...
        if(validLists.contains(dr)){
          ...
          if(kvparrayopt.isDefined){
            ...
            val filteredList = kvparray.filter(!_.values.toSet.intersect(indexValues).isEmpty)
  ...
  def getEscapedValue(value:String) = if(!value.trim.isEmpty) "_" + value else value.trim
...
object TaxonProfileDAO {
  ...
  def createTaxonProfile(map: Option[Map[String, String]]): TaxonProfile = {
    ...
    map.get.foreach(keyValue => {
      keyValue._1 match {
        ...
        case "habitats" => {
          if (keyValue._2 != null && !keyValue._2.isEmpty) {
  ...
  def add(taxonProfile:TaxonProfile) {
    ...
    if(taxonProfile.sensitive != null && !taxonProfile.sensitive.isEmpty){
